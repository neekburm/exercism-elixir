defmodule IsbnVerifier do
  ...
  def parse_isbn_char(?X), do: 10
  def parse_isbn_char(char) when char in ?0..?9, do: List.to_integer([char])
  def parse_isbn_char(?\-), do: nil
  def parse_isbn_char(char), do: char

  def valid_isbn?(isbn_numbers) do
